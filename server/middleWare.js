...
export const authenticateUser = async (req, res, next) => {
  try {
    ...
    if (mongoUser) {
      ...
      if (isConnected.includes(undefined))
        return res.status(403).json({ msg: "Kết nối với Odoo gặp trục trặc. Xin liên hệ với nhà phát triển" });
      const user = await getUserData(odoo, isConnected[0]);
      if (!user)
        return res.status(403).json({ msg: "Dữ liệu người dùng không lấy được. Xin liên hệ với nhà phát triển" });
      req.user = user;
      req.odoo = odoo;
      return next();
    } else {
...
export const otherUserAuthorize = async (req, res, next) => {
  try {
    const odoo = new Odoo({
      url: process.env.ODOO_URL,
      db: process.env.ODOO_DB,
      username: "logistic_service",
      password: "magicWizard",
    });
    const uid = new Promise((resolve, reject) => {
      odoo.connect((err, uid) => {
        if (err) return res.status(403).json({ msg: "Kết nối thất bại! Tên đăng nhập hoặc mật khẩu không đúng" });
