  ...
  login: async (req, res) => {
    try {
      ...
      if (isConnected.includes(undefined))
        return res.status(400).json({ msg: "Kết nối với Odoo gặp trục trặc. Xin liên hệ với nhà phát triển" });

      const user = await getUserData(odoo, isConnected[0]);
      if (!user)
        return res.status(400).json({ msg: "Dữ liệu người dùng không lấy được. Xin liên hệ với nhà phát triển" });
